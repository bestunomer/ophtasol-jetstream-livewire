          ...
          <article>
			...
			<div class="span7 droid" align='right' >
                <div class="project-widget droid" >
                  <h4 class='droid' style='font-size:18px'> معلومات عن الطبیب<i class="icon-48 icon-beaker"></i></h4>
                  ...
                  if(isset($_REQUEST["did"]))
               {
				...
				$row=$result->fetch(PDO::FETCH_ASSOC);  
				    ...
				    if(!empty($row['fullname_a']))
				{
                   print"<li><label>الاسم:</label> ".$row['fullname_a']."</li>";
				}
				   if(!empty($row['qualification_a']))
				{
                   print"<li><label>الشهادة :</label> ".$row['qualification_a']."</li>";
				}
				   if(!empty($row['doctor_cat_A']))
				{
                   print"<li><label>الاختصاص :</label> ".$row['doctor_cat_A']."</li>";
				}
                if(!empty($row['work_mobile']))
				{
                   print"<li dir='rtl'><label>الهاتف :</label> ".$row['work_mobile']."</li>";
				}
				 if(!empty($row['work_email']))
				{
                   print"<li dir='rtl'><label>ایمیل :</label> <a href=mailto:".$row['work_email'].">".$row['work_email']."</a></li>";
				}

				if(!empty($row['working_hours_a']))
				{  
                   print"<li dir='rtl'><label>اوقات الدوام:</label> ".$row['working_hours_a']."</li>";
				}
				   if(!empty($row['clinic_address_k']))
				{
				   print"<li dir='rtl'><label>عنوان العیادة الطبیة:</label> ".$row['clinic_address_a']."</li>";
				}
				   if(!empty($row['clinic_mobile']))
				{
				   print"<li dir='rtl'><label>رقم هاتف العیادة الطبیة:</label> ".$row['clinic_mobile']."</li>";
				}
				   if(!empty($row['video_link']))
				{
                   print"<li dir='rtl'><label>فیدیو :</label><a href=".$row['video_link']." target=_blank> ".$row['video_link']."</a></li>";
				}
             ...
             <?php
                ...
                $count = $result->rowCount();
                  ...
                  { 
                        ...
                        print"<p class='droid' align='right'>".$row['detail_about_doctor_a']."</p>";
