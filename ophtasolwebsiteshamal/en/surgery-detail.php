...
<head>
  <meta charset="utf-8" />
  <title>Ibinsina Hospital - <?php if(isset($_REQUEST["suid"]))
               {
                 $suid=$_REQUEST["suid"];
                 if(is_numeric($suid))
                 {
                    $result = $dbh->prepare("SELECT * from surgery where language_id=1 and publish_id=1 and surgery_id=:suid");
              ...
              if(isset($_REQUEST["suid"]))
               {
                ...
                if ($count > 0) 
                { 
                    print"<h3 align='left'><a href='surgery-detail.php?suid=".$row['surgery_id=']."'>".$row['title']."</a></h3>";

                       print"<a href='surgery-detail.php?suid=".$row['surgery_id=']."'><img src=../assets/img/surgery/".$row['photo']." /></a>";
...
<?php
                  ...
                  $result = $dbh->prepare("SELECT * from surgery where language_id=1 and publish_id=1");
                  $result->execute();
                  $count = $result->rowCount();
                if ($count > 0) 
                { 
			 ...
			 echo"  <li><a href='surgery-detail.php?suid=".$row['surgery_id']."'>".$row['title']."</a></li>";
